...
describe('PullRequestCard', () => {
  it('renders the approved single-line token order with distinct links and explicit separators', () => {
    ...
    expect(visibleCardText()).toBe('2 unresolved · −81/+340 · 12 files · Codex 3 · Claude 0');
  ...
  it('renders a ready empty agent section as definitive', () => {
    ...
    expect(visibleCardText()).toBe('2 unresolved · −81/+340 · 12 files · No AI agents');
  });
  ...
  it('uses singular grammar and exposes extension error reasons to assistive technology', () => {
    ...
    expect(visibleCardText()).toBe('1 unresolved · −1/+1 · 1 file · Codex 3 · Claude 0');
    ...
    expect(visibleCardText()).toBe('— unresolved · — files · AI agents unavailable');
    expect(screen.getByText('— unresolved')).not.toHaveClass('unresolved');
    for (const reason of ['threads failed', 'files failed', 'agents failed']) {
  ...
  it('updates aria-busy and retains one stable status node as sections load', () => {
    const loading: PullRequestSummary = { ...ready, reviewThreads: { status: 'loading' } };
