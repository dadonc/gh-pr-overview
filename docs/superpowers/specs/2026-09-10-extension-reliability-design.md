...
- Keep the existing card layout. Show `≥N unresolved` and `Agent ≥N` for partial thread and agent counts; retain explanatory hover and accessible descriptions.
...
- Show an accessible Retry button when a card has an error or partial data. Retry bypasses cached results, never overlaps the row's active load, and preserves completed sections while work is in progress. Use a refreshing flag to disable repeated manual retries.
